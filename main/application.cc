    ...
    protocol_->OnIncomingJson([this, display](const cJSON* root) {
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (cJSON_IsString(text)) {
                ...
                std::string lower_text = stt_text;
                for (char &c : lower_text) {
                    if (c >= 'A' && c <= 'Z') {
                        c = c + 32;
                    }
                }
                if (lower_text.find("tạm biệt") != std::string::npos || 
                    lower_text.find("bật đồng hồ") != std::string::npos ||
                    lower_text.find("mở đồng hồ") != std::string::npos) {
                    trigger_clock_on_idle_ = true;
                }
...
void Application::HandleStateChangedEvent() {
    ...
    switch (new_state) {
        ...
        case kDeviceStateIdle:
            ...
            audio_service_.EnableWakeWordDetection(true);
            if (trigger_clock_on_idle_) {
                trigger_clock_on_idle_ = false;
                display->TriggerClockScreen();
            }
            break;
