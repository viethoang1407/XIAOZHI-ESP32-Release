...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    bool is_youtube_play = (strcmp(role, "assistant") == 0 && strstr(content, "https://www.youtube.com/watch?v=") != nullptr);

    if (is_youtube_play) {
        // Hide default text label
        lv_obj_add_flag(msg_text, LV_OBJ_FLAG_HIDDEN);

        // Extract song title
        std::string song_title = "YouTube Music";
        const char* start_ptr = strstr(content, "trình duyệt máy tính của bạn: ");
        if (start_ptr != nullptr) {
            start_ptr += strlen("trình duyệt máy tính của bạn: ");
            const char* end_ptr = strstr(start_ptr, " - https://");
            if (end_ptr != nullptr) {
                song_title = std::string(start_ptr, end_ptr - start_ptr);
            } else {
                end_ptr = strstr(start_ptr, "https://");
                if (end_ptr != nullptr) {
                    song_title = std::string(start_ptr, end_ptr - start_ptr);
                }
            }
        ...
        // Configure custom bubble container
        lv_obj_set_width(msg_bubble, max_width);
        lv_obj_set_flex_flow(msg_bubble, LV_FLEX_FLOW_COLUMN);
        lv_obj_set_style_pad_all(msg_bubble, lvgl_theme->spacing(4), 0);
        lv_obj_set_style_bg_color(msg_bubble, lv_color_make(18, 18, 24), 0);
        lv_obj_set_style_bg_opa(msg_bubble, LV_OPA_90, 0);
        lv_obj_set_style_border_width(msg_bubble, 1, 0);
        lv_obj_set_style_border_color(msg_bubble, lv_color_make(45, 45, 60), 0);
        ...
        // 1. Header row
        lv_obj_t* header_row = lv_obj_create(msg_bubble);
        ...
        lv_obj_t* music_icon = lv_label_create(header_row);
        lv_obj_set_style_text_font(music_icon, icon_font, 0);
        lv_obj_set_style_text_color(music_icon, lv_color_make(0, 229, 255), 0);
        lv_label_set_text(music_icon, FONT_AWESOME_MUSIC);

        lv_obj_t* brand_text = lv_label_create(header_row);
        lv_obj_set_style_text_font(brand_text, text_font, 0);
        lv_obj_set_style_text_color(brand_text, lv_color_make(150, 150, 150), 0);
        lv_label_set_text(brand_text, " YouTube Music");

        // 2. Song Title
        lv_obj_t* title_label = lv_label_create(msg_bubble);
        lv_obj_set_width(title_label, LV_PCT(100));
        lv_label_set_long_mode(title_label, LV_LABEL_LONG_WRAP);
        lv_obj_set_style_text_font(title_label, text_font, 0);
        lv_obj_set_style_text_color(title_label, lv_color_white(), 0);
        lv_label_set_text(title_label, song_title.c_str());
        lv_obj_set_style_margin_top(title_label, lvgl_theme->spacing(2), 0);

        // 3. Progress Bar
        lv_obj_t* progress_bar = lv_bar_create(msg_bubble);
        lv_obj_set_size(progress_bar, LV_PCT(100), 4);
        lv_obj_set_style_bg_color(progress_bar, lv_color_make(60, 60, 60), LV_PART_MAIN);
        lv_obj_set_style_bg_color(progress_bar, lv_color_make(0, 229, 255), LV_PART_INDICATOR);
        lv_bar_set_value(progress_bar, 35, LV_ANIM_OFF);
        lv_obj_set_style_margin_top(progress_bar, lvgl_theme->spacing(3), 0);

        // 4. Controls Row
        lv_obj_t* controls_row = lv_obj_create(msg_bubble);
        ...
        lv_obj_set_style_margin_top(controls_row, lvgl_theme->spacing(3), 0);
        lv_obj_set_style_pad_column(controls_row, lvgl_theme->spacing(6), 0);

        lv_obj_t* prev_btn = lv_label_create(controls_row);
        lv_obj_set_style_text_font(prev_btn, icon_font, 0);
        lv_obj_set_style_text_color(prev_btn, lv_color_make(200, 200, 200), 0);
        lv_label_set_text(prev_btn, FONT_AWESOME_BACKWARD_STEP);

        lv_obj_t* play_btn = lv_label_create(controls_row);
        lv_obj_set_style_text_font(play_btn, large_icon_font ? large_icon_font : icon_font, 0);
        lv_obj_set_style_text_color(play_btn, lv_color_make(0, 229, 255), 0);
        lv_label_set_text(play_btn, FONT_AWESOME_PAUSE);

        lv_obj_t* next_btn = lv_label_create(controls_row);
        lv_obj_set_style_text_font(next_btn, icon_font, 0);
        lv_obj_set_style_text_color(next_btn, lv_color_make(200, 200, 200), 0);
        lv_label_set_text(next_btn, FONT_AWESOME_FORWARD_STEP);
...
void LcdDisplay::DismissClockScreen() {
    DisplayLockGuard lock(this);
    if (is_clock_screen_active_) {
        is_clock_screen_active_ = false;
        if (clock_screen_obj_ != nullptr) {
            lv_obj_add_flag(clock_screen_obj_, LV_OBJ_FLAG_HIDDEN);
        }
    }
    last_activity_time_ = std::chrono::system_clock::now();
}

void LcdDisplay::TriggerClockScreen() {
    DisplayLockGuard lock(this);
    if (!is_clock_screen_active_) {
        is_clock_screen_active_ = true;
        CreateClockScreen();
        if (clock_screen_obj_ != nullptr) {
            lv_obj_remove_flag(clock_screen_obj_, LV_OBJ_FLAG_HIDDEN);
        }
        UpdateClockScreenData();
    }
}
...
// -----------------------------------------------------------------------------
// Clock Screen & Weather/RSS Ticker Implementation
// -----------------------------------------------------------------------------

void LcdDisplay::UpdateStatusBar(bool update_all) {
    // Call base class UpdateStatusBar first
    ...
    if (state != kDeviceStateIdle) {
        // Device is active/listening/speaking: hide clock screen
        if (is_clock_screen_active_) {
            DisplayLockGuard lock(this);
            is_clock_screen_active_ = false;
            if (clock_screen_obj_ != nullptr) {
        ...
        // Device is idle
        if (!is_clock_screen_active_) {
            // Wait for 15 seconds of inactivity before showing the clock face
            if (last_activity_time_ + std::chrono::seconds(15) < std::chrono::system_clock::now()) {
                DisplayLockGuard lock(this);
                is_clock_screen_active_ = true;
                CreateClockScreen();
                if (clock_screen_obj_ != nullptr) {
                ...
                StartWeatherAndRssFetch();
                UpdateClockScreenData();
            }
        } else {
            // Clock screen is active: update time digits
            DisplayLockGuard lock(this);
            UpdateClockScreenData();

            // Refresh weather and RSS feed every 30 minutes
            auto now = std::chrono::system_clock::now();
            if (last_weather_fetch_time_ == std::chrono::system_clock::time_point() ||
                last_weather_fetch_time_ + std::chrono::minutes(30) < now) {
                StartWeatherAndRssFetch();
            }
        }
    }
...
void LcdDisplay::CreateClockScreen() {
    if (clock_screen_obj_ != nullptr) {
        return;
    }
    ...
    clock_screen_obj_ = lv_obj_create(screen);
    lv_obj_set_size(clock_screen_obj_, LV_HOR_RES, LV_VER_RES);
    lv_obj_set_style_radius(clock_screen_obj_, 0, 0);
    lv_obj_set_style_border_width(clock_screen_obj_, 0, 0);
    lv_obj_set_style_bg_color(clock_screen_obj_, lv_color_hex(0x0a0a0c), 0);
    lv_obj_set_style_pad_all(clock_screen_obj_, 0, 0);
    lv_obj_align(clock_screen_obj_, LV_ALIGN_CENTER, 0, 0);
    lv_obj_add_flag(clock_screen_obj_, LV_OBJ_FLAG_HIDDEN);

    // Event listener: tap to wake up (dismiss clock screen)
    lv_obj_add_event_cb(clock_screen_obj_, [](lv_event_t* e) {
        LcdDisplay* self = (LcdDisplay*)lv_event_get_user_data(e);
        self->Lock();
        self->is_clock_screen_active_ = false;
        if (self->clock_screen_obj_ != nullptr) {
            lv_obj_add_flag(self->clock_screen_obj_, LV_OBJ_FLAG_HIDDEN);
        }
        self->last_activity_time_ = std::chrono::system_clock::now();
        self->Unlock();
    }, LV_EVENT_CLICKED, this);

    // Flex layout
    lv_obj_t* layout = lv_obj_create(clock_screen_obj_);
    ...
    lv_obj_set_style_pad_row(layout, 15, 0); // 15px vertical space between items
    lv_obj_align(layout, LV_ALIGN_CENTER, 0, 0);

    // Top: Header Widget (originally weather widget)
    lv_obj_t* weather_card = lv_obj_create(layout);
    lv_obj_set_size(weather_card, LV_HOR_RES - 20, 45);
    lv_obj_set_style_radius(weather_card, 10, 0);
    lv_obj_set_style_bg_color(weather_card, lv_color_hex(0x1a1a22), 0);
    lv_obj_set_style_bg_opa(weather_card, LV_OPA_70, 0);
    lv_obj_set_style_border_width(weather_card, 0, 0);
    lv_obj_set_style_pad_top(weather_card, 5, 0);
    lv_obj_set_style_pad_bottom(weather_card, 5, 0);
    lv_obj_set_style_pad_left(weather_card, 10, 0);
    lv_obj_set_style_pad_right(weather_card, 10, 0);
    ...
    clock_weather_label_ = lv_label_create(weather_card);
    lv_obj_set_style_text_font(clock_weather_label_, text_font, 0);
    lv_obj_set_style_text_color(clock_weather_label_, lv_color_hex(0x00E5FF), 0);
    lv_label_set_long_mode(clock_weather_label_, LV_LABEL_LONG_CLIP);
    lv_obj_set_width(clock_weather_label_, LV_HOR_RES - 110);
    lv_label_set_text(clock_weather_label_, "Hi Lili AI");

    // Right-aligned status (WiFi and Battery)
    lv_obj_t* status_container = lv_obj_create(weather_card);
    ...
    clock_wifi_icon_ = lv_label_create(status_container);
    lv_obj_set_style_text_font(clock_wifi_icon_, icon_font, 0);
    lv_obj_set_style_text_color(clock_wifi_icon_, lv_color_hex(0x00FF00), 0);
    lv_label_set_text(clock_wifi_icon_, "");
    lv_obj_set_style_margin_right(clock_wifi_icon_, 8, 0);

    clock_battery_icon_ = lv_label_create(status_container);
    lv_obj_set_style_text_font(clock_battery_icon_, icon_font, 0);
    lv_obj_set_style_text_color(clock_battery_icon_, lv_color_hex(0xFFFFFF), 0);
    lv_label_set_text(clock_battery_icon_, "");
    lv_obj_set_style_margin_right(clock_battery_icon_, 4, 0);

    clock_battery_text_ = lv_label_create(status_container);
    lv_obj_set_style_text_font(clock_battery_text_, text_font, 0);
    lv_obj_set_style_text_color(clock_battery_text_, lv_color_hex(0xaaaaaa), 0);
    lv_label_set_text(clock_battery_text_, "");

    // Center: Digital Time & Date
    lv_obj_t* time_card = lv_obj_create(layout);
    lv_obj_set_size(time_card, LV_HOR_RES - 20, 110);
    lv_obj_set_style_radius(time_card, 15, 0);
    lv_obj_set_style_bg_color(time_card, lv_color_hex(0x14141c), 0);
    lv_obj_set_style_bg_opa(time_card, LV_OPA_80, 0);
    lv_obj_set_style_border_color(time_card, lv_color_hex(0x2d2d3d), 0);
    ...
    clock_time_label_ = lv_label_create(time_card);
    ...
    lv_label_set_text(clock_time_label_, "00:00:00");

    clock_date_label_ = lv_label_create(time_card);
    ...
    lv_label_set_text(clock_date_label_, "Đang tải ngày tháng...");
}

void LcdDisplay::UpdateClockScreenData() {
    ...
    if (tm->tm_year >= 2025 - 1900) {
        ...
        char date_str[128];
        snprintf(date_str, sizeof(date_str), "%s, ngày %02d/%02d/%d", wdays[tm->tm_wday], tm->tm_mday, tm->tm_mon + 1, tm->tm_year + 1900);
        lv_label_set_text(clock_date_label_, date_str);
    } else {
        lv_label_set_text(clock_time_label_, "--:--:--");
        lv_label_set_text(clock_date_label_, "Đang đồng bộ NTP...");
    }

    if (clock_weather_label_ != nullptr) {
        const char* cur = lv_label_get_text(clock_weather_label_);
        if (cur == nullptr || strcmp(cur, "Trợ lý ảo Xiaozhi") != 0) {
            lv_label_set_text(clock_weather_label_, "Trợ lý ảo Xiaozhi");
        }
    }

    auto& board = Board::GetInstance();
    
    // Update WiFi status icon on Clock Screen
    const char* wifi_icon = board.GetNetworkStateIcon();
    if (clock_wifi_icon_ != nullptr && wifi_icon != nullptr) {
        lv_label_set_text(clock_wifi_icon_, wifi_icon);
        // Green for wifi connected, red if disconnected
        if (strcmp(wifi_icon, FONT_AWESOME_WIFI_SLASH) == 0 || strcmp(wifi_icon, FONT_AWESOME_SIGNAL_OFF) == 0) {
            lv_obj_set_style_text_color(clock_wifi_icon_, lv_color_hex(0xFF3B30), 0); // Red
        } else {
            lv_obj_set_style_text_color(clock_wifi_icon_, lv_color_hex(0x00FF00), 0); // Green
        }
    }

    // Update Battery level on Clock Screen
    int battery_level = 0;
    bool charging = false;
    ...
    if (board.GetBatteryLevel(battery_level, charging, discharging)) {
        ...
        if (clock_battery_text_ != nullptr) {
            lv_label_set_text(clock_battery_text_, bat_str);
        }
        ...
        if (clock_battery_icon_ != nullptr && icon != nullptr) {
            lv_label_set_text(clock_battery_icon_, icon);
            if (charging) {
                lv_obj_set_style_text_color(clock_battery_icon_, lv_color_hex(0x00FF00), 0); // Green
            } else if (battery_level < 20) {
                lv_obj_set_style_text_color(clock_battery_icon_, lv_color_hex(0xFF3B30), 0); // Red
            } else {
                lv_obj_set_style_text_color(clock_battery_icon_, lv_color_hex(0xFFFFFF), 0); // White
            }
        }
    } else {
        if (clock_battery_text_ != nullptr) {
        ...
        if (clock_battery_icon_ != nullptr) {
            lv_label_set_text(clock_battery_icon_, FONT_AWESOME_BATTERY_SLASH);
            lv_obj_set_style_text_color(clock_battery_icon_, lv_color_hex(0x8E8E93), 0);
        }
    }
}

void LcdDisplay::SetWeatherAndRss(const std::string& weather, const std::string& rss) {
    // Disabled
}

void LcdDisplay::StartWeatherAndRssFetch() {
    // Disabled weather and RSS fetching to avoid WiFi disconnection issues
}
